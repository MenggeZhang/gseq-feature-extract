...
def main():
    ...
    if (numCmdArgs != 4):
        print('Correct Usage:')
        print('python3 CRC_Logistic.py mc kmer Epi')
        sys.exit(0)
    ...
    for ft in range(0,4):
        Output_List_File = open('Logistic_F'+str(ft+1)+'_k'+kmer+'_m'+mc+'.csv', 'w')
        OutputMatrix = []
        ...
        for i in range(0,3):
            ...
            reg = LogisticRegressionCV(cv=5, random_state=None, penalty ='l2',class_weight ='balanced', multi_class='ovr').fit(Total_X[i][ft][:num_training], Total_Y[i][ft][:num_training])
            predicted_Y_Logistic = (reg.predict_proba(Total_X[i][ft][num_training:]))[:,1]
            # print(reg.classes_)
            # print(Total_Y[i][ft][num_training:])
            # print(predicted_Y_Logistic[:,1])
            fpr_Logistic, tpr_Logistic, _ = roc_curve(Total_Y[i][ft][num_training:], predicted_Y_Logistic)
            auc_Logistic = auc(fpr_Logistic, tpr_Logistic)
            OutputMatrix.append([Total_Datasets_Names[i],Total_Datasets_Names[i],str(auc_Logistic)])
        
        # Use one dataset for training, test on the other two datasets
        Combination_1 = [[0,1],[0,2],[1,0],[1,2],[2,0],[2,1]]
        for vec in Combination_1:
            reg = LogisticRegressionCV(cv=5, random_state=None, penalty ='l2',class_weight ='balanced', multi_class='ovr').fit(Total_X[vec[0]][ft],Total_Y[vec[0]][ft])
            predicted_Y_Logistic = (reg.predict_proba(Total_X[vec[1]][ft]))[:,1]
            fpr_Logistic, tpr_Logistic, _ = roc_curve(Total_Y[vec[1]][ft], predicted_Y_Logistic)
            auc_Logistic = auc(fpr_Logistic, tpr_Logistic)
            OutputMatrix.append([Total_Datasets_Names[vec[0]],Total_Datasets_Names[vec[1]],str(auc_Logistic)])

        # Use two dataset for training, test on the other dataset
        Combination_2 = [[0,1,2],[0,2,1],[1,2,0]]
        for vec in Combination_2:
            reg = LogisticRegressionCV(cv=5, random_state=None, penalty ='l2',class_weight ='balanced', multi_class='ovr').fit(Total_X[vec[0]][ft]+Total_X[vec[1]][ft],Total_Y[vec[0]][ft]+Total_Y[vec[1]][ft])
            predicted_Y_Logistic = (reg.predict_proba(Total_X[vec[2]][ft]))[:,1]
            fpr_Logistic, tpr_Logistic, _ = roc_curve(Total_Y[vec[2]][ft], predicted_Y_Logistic)
            auc_Logistic = auc(fpr_Logistic, tpr_Logistic)
            OutputMatrix.append([Total_Datasets_Names[vec[0]]+'_'+Total_Datasets_Names[vec[1]],Total_Datasets_Names[vec[2]],str(auc_Logistic)])
